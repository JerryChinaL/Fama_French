...
cor_matrix_five_factors <- factors_filtered %>%
  select(MKT_R = r_mkt, SMB, HML, RMW, CMA) %>%
  cor(use = "pairwise.complete.obs")

# Panel C: Correlation Matrix for the SMB Sub-Factors
cor_matrix_SMB_factors <- factors_filtered %>%
  select(SMB_bm, SMB_op, SMB_inv, SMB) %>%
  cor(use = "pairwise.complete.obs")
...
\\hline
\\multicolumn{6}{l}{Panel A: Summary statistics of the five factors} \\\\
 & $MKT_R$ & $SMB$ & $HML$ & $RMW$ & $CMA$ \\\\
 \\hline
Arithmetic mean & ", paste(summary_stats_table_formatted$MKT_R[1], summary_stats_table_formatted$SMB[1], summary_stats_table_formatted$HML[1], summary_stats_table_formatted$RMW[1], summary_stats_table_formatted$CMA[1], sep = " & "), " \\\\
Standard deviation & ", paste(summary_stats_table_formatted$MKT_R[2], summary_stats_table_formatted$SMB[2], summary_stats_table_formatted$HML[2], summary_stats_table_formatted$RMW[2], summary_stats_table_formatted$CMA[2], sep = " & "), " \\\\
T-statistic & ", paste(summary_stats_table_formatted$MKT_R[3], summary_stats_table_formatted$SMB[3], summary_stats_table_formatted$HML[3], summary_stats_table_formatted$RMW[3], summary_stats_table_formatted$CMA[3], sep = " & "), " \\\\
\\\\
\\multicolumn{6}{l}{Panel B: Correlation matrix for the five factors} \\\\
 & $MKT_R$ & $SMB$ & $HML$ & $RMW$ & $CMA$ \\\\
 \\hline
...
\\\\
\\multicolumn{6}{l}{Panel C: Correlation matrix for the SMB sub-factors} \\\\
 & $SMB_{bm}$ & $SMB_{op}$ & $SMB_{inv}$ & $SMB$ \\\\
 \\hline
