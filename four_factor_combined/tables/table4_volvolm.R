...
momentum <- readRDS("../../ELM/data/ELM-4-factors-replicated-nordq2.rds") %>%
  select(monthly_date, VOLM = r_volm)
...
for (var in variables) {
  ...
  saveRDS(joined_data, paste0("tables/data/table6_volvolm_", var, "_ret.rds"))
  
  # Perform regressions for each quantile pair
  reg_results <- lapply(names(joined_data)[2:26], function(quantile_col) {
    formula <- as.formula(paste(quantile_col, "~ r_mkt + SMB + HML + RMW + CMA + VOLM"))
    lm(formula, data = joined_data)
  })
  
  # Extract coefficients and organize into matrices
  coef_names <- c("(Intercept)", "r_mkt", "SMB", "HML", "RMW", "CMA", "VOLM")
...
output_file <- "tables/regression_results_volvolm.tex"
...
for (coef_name in c("(Intercept)", "r_mkt", "SMB", "HML", "RMW", "CMA", "VOLM")) {
