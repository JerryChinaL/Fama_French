...
# Most recent size is used in BM calculation
...
combined_df <- book_equity_df %>%
  inner_join(size, by = c("KYGVKEY", "sort_date")) %>%
  mutate(bm = be1 / me)
...
write.csv(four_factors_excret, "data/four_factors_excret_lag1.csv", row.names = FALSE)
