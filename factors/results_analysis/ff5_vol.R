...
exch <- readRDS("../../ELM/data/sfz_agg_mth_short.rds") %>% 
  select(KYPERMNO, YYYYMM, PRIMEXCH, MTHVOL, MTHPRC) %>%
  mutate(VOL = MTHVOL * MTHPRC,
         monthly_date = as.Date(paste0(YYYYMM, "01"), format = "%Y%m%d"))

exch <- exch %>%
  select(KYPERMNO, monthly_date, VOL) %>%
  mutate(monthly_date = monthly_date + months(1)) %>%
  right_join(exch %>% select(-VOL), by = c("KYPERMNO", "monthly_date")) %>%
  select(PRIMEXCH, VOL, YYYYMM, KYPERMNO)
  
primiss <- readRDS("data/primiss.rds") %>%
  mutate(
...
mthret <- mthret %>%
  ...
  inner_join(primiss, by = c("KYPERMNO", "KYGVKEY", "YYYYMM")) %>% # use primary stock
  mutate(
  ...
  select(KYPERMNO, KYGVKEY, VOL, sort_date, MTHRET, return_date = MCALDT, PRIMEXCH, YYYYMM)
...
portfolios <- final_df_extended %>%
  ...
  select(-YYYYMM)
# %>%
  # select(KYPERMNO, KYGVKEY, sort_date, return_date, portfolio_size, portfolio_bm,portfolio_op, portfolio_inv, portfolio_vol, VOL, SIZE)
...
# %>%
  # select(KYPERMNO, KYGVKEY, YYYYMM, MTHRET, portfolio_size, portfolio_bm, portfolio_op, portfolio_inv,portfolio_vol, VOL, SIZE)
...
saveRDS(portfolios_w_return, "data/portfolios_w_return_vol.rds")
...
write.csv(factors_replicated, "data/ff5_vol.csv", row.names = FALSE)
